...
| Database Storage Layer | SQLite database named `forensics.db` |
...
1. **LSTM Model Scoring**
   - Uses the existing `attack_detection_model.h5`
   - Uses `scaler.pkl`
...
The features are scaled using `scaler.pkl`, reshaped for LSTM input, and passed into the Keras model.
